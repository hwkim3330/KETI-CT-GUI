...
export class MUP1Protocol {
    ...
    encodeFrame(type, data = Buffer.alloc(0)) {
        // Build un-escaped frame for checksum calculation (like Ruby implementation)
        const frameForChecksum = [];
        frameForChecksum.push(this.SOF);
        frameForChecksum.push(type);
        frameForChecksum.push(...data);
        frameForChecksum.push(this.EOF);

        // Add second EOF if ORIGINAL data size is even (for alignment)
        // This matches Ruby: frame_a << MUP1_EOF if data.size % 2 == 0
        if (data.length % 2 === 0) {
            frameForChecksum.push(this.EOF);
        }

        // Calculate checksum on un-escaped frame
        const checksum = this.calculateChecksum(Buffer.from(frameForChecksum));
        const checksumStr = checksum.toString(16).toUpperCase().padStart(4, '0');

        // Now build the actual escaped frame to send
        const frame = [];
        ...
        // 4. End of frame (with padding based on ORIGINAL data size)
        frame.push(this.EOF);
        if (data.length % 2 === 0) {
            frame.push(this.EOF);
        }
    ...
    /**
     * Calculate 16-bit one's complement checksum (matching Ruby implementation)
     * @param {Buffer} data - Data to checksum (frame from SOF to EOF inclusive)
    ...
    calculateChecksum(data) {
        ...
        // Matches Ruby: data.pack("C*").unpack("n*").sum
        for (let i = 0; i < data.length; i += 2) {
            if (i + 1 < data.length) {
                // Both bytes available - network byte order (big-endian)
                sum += (data[i] << 8) | data[i + 1];
            } else {
                // Odd byte: treat as high byte with 0x00 low byte
                sum += data[i] << 8;
            }
        }

        // Add carry twice (Ruby implementation)
